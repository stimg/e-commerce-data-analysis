...
def calculate_revenue_by_state(sales_data: pd.DataFrame,
    ...
    ----------
    sales_data : pd.DataFrame
        Sales data with 'customer_id' and 'price' columns.
    ...
    # Merge sales with customer state info
    sales_with_states = sales[['customer_id', 'price']].merge(
        cust[['customer_id', 'customer_state']],
        on='customer_id',
        how='left'
    )
    ...
    result = pd.DataFrame({
        'customer_state': revenue_by_state.index,
        'price': revenue_by_state.values
    })
...
def calculate_order_status_distribution(sales_data: pd.DataFrame,
    ...
    ----------
    sales_data : pd.DataFrame
        Sales dataset with 'order_status' and optionally 'year' columns.
    year : int, optional
    ...
    return unique_orders['order_status'].value_counts(normalize=True).sort_values(ascending=False)
